...
std::vector<State> MotionPlanner::generate_offset_goals(
  ...
  auto yaw = goal_state.rotation.yaw + (M_PI / 2);  // <- Fix This // KZOH Done
  ...
  for (int i = 0; i < _num_paths; ++i) {
    ...
    goal_offset.location.x += offset * std::cos(yaw);  // <- Fix This // KZOH Done
    goal_offset.location.y += offset * std::sin(yaw);  // <- Fix This // KZOH Done
