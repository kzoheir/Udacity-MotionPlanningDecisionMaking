...
double BehaviorPlannerFSM::get_look_ahead_distance(const State& ego_state) {
  ...
  // KZOH: v^2 = vo ^2 + 2 * a * s; v=0; Notes: a (-ve) and v = 0; 
  // KZOH: s = (vo^2) / (2 * a)
...
State BehaviorPlannerFSM::state_transition(const State& ego_state, State goal,
  ...
  if (_active_maneuver == FOLLOW_LANE) {
    ...
    if (is_goal_in_junction) {
      ...
      goal.location.x += _stop_line_buffer * std::cos(ang);  // <- Fix This // KZOH Done
      goal.location.y += _stop_line_buffer * std::sin(ang);  // <- Fix This  // KZOH Done

      // LOG(INFO) << "BP- new STOP goal at: " << goal.location.x << ", "
      //          << goal.location.y;

      // TODO-goal speed at stopping point: What should be the goal speed??
      goal.velocity.x = 0.0;  // <- Fix This //KZOH - Done
      goal.velocity.y = 0.0;  // <- Fix This //KZOH - Done
      goal.velocity.z = 0.0;  // <- Fix This //KZOH - Done
      ...
      goal.velocity.x = _speed_limit * std::cos (goal.rotation.yaw);  // <- Fix This //KZOH - Done
      goal.velocity.y = _speed_limit * std::sin (goal.rotation.yaw);  // <- Fix This //KZOH - Done
      goal.velocity.z = 0.0;
    }
    ...
    // if (utils::magnitude(ego_state.velocity) <=
        // _stop_threshold_speed) {  // -> Fix this //KZOH - Done
    ...
    // the stop line. goal = ...;
       goal = _goal;  // Keep previous goal. Stay where you are. // <- Fix This //KZOH - Done
